...
collision_correction_factor = 0.8
...
class POLYGON:
    ...
    def getMTV(P1, P2):
        ...
        reverse, ID, reverseID = False, 0, len(P1Edge)
        contact_edge_P1 = []
        contact_edge_P2 = []
        for edge in allEdge:
            if abs(abs(edge.getVT() | MTV) - 1.0) < collision_parallel_error:
                if ID == reverseID:
                    reverse = True
                (contact_edge_P2 if reverse else contact_edge_P1).append(edge)
            ID += 1
        return MTV
...
class RIGID_BODY:
    ...
    def rebound(B1, B2, collision, e):
        ...
        inv_inertia_sum = R1_cross_n ** 2 / R2_cross_n ** 2 / B2.inertia
